...
add_action( 'admin_menu', 'dp_createMenu' );
add_action( 'admin_init', 'dp_registerSettings' );
add_action( 'admin_init', 'dp_getTelegramMessages' );
add_action( 'admin_enqueue_scripts', 'dp_register_styles' );
$form_id = dp_get_form_id();
add_action( "gform_after_submission_$form_id",'dp_callAPI', 10, 4 );
}
...
function dp_register_styles(){
	...
	wp_register_style('bootstrap_admin_theme_css', plugin_dir_url( __FILE__ ) . '/css/bootstrap.min.css');
	wp_register_style('bootstrap_admin_theme', plugin_dir_url( __FILE__ ) . '/js/bootstrap.bundle.min.js');
	...
	wp_enqueue_style('bootstrap_admin_theme_css', plugin_dir_url( __FILE__ ) . '/css/bootstrap.min.css');
	...
	wp_enqueue_script('bootstrap_admin_theme', plugin_dir_url( __FILE__ ) . '/js/bootstrap.bundle.min.js');
...
function dp_createMenu() {
	...
	add_menu_page( 'DoliPress', 'DoliPress', 'manage_options', 'doligravity-settings', 'dp_settingsPage', $icon  );
	}

//Function to generate the settings field

function dp_registerSettings(){
...
function dp_settingsfields(){	 	
	
	add_option('dpapikey','');
	add_option('dgurl','');
	add_option('dgform','');
	add_option('usehttps','');
	add_option('dp_errors','');
	...
	add_settings_field( 'usehttps','Use Localhost/HTTPS','dp_use_https','doligravity-settings','doligravity_settings');
	...
	dp_exception_handler();
}

//Manage Exception for empty URL field

function dp_exception_handler(){
...
function dp_callAPI( $entry, $form, $fieldscount ){
...
//Print Errors Array
function dp_print_errors(){
	$dp_errors = get_option('dp_errors');
	if ($dp_errors != ''){
      foreach ($dp_errors as $value){
          echo $value . '<br>';
      }
	}
	else {
